...
sudo pacman -S --verbose --noconfirm base-devel fakeroot gcc guilw binutils groff gettext sudo texinfo pacman gawk bison autoconf automake elfutils make findutils libtool file flex m4 grep sed gc patch gzip fakeroot libmpc pkgconf which lib32-fakeroot visudo autoconf automake bison flex m4 cmake make diffutils gcc lib32-gcc-libs manjaro-tools-base manjaro-tools-pkg papirus-icon-theme papirus-folders


##This next line is entirely up to you. Change it as you wish.
sudo pacman -S --verbose --noconfirm rsync yay gufw bitwarden telegram-desktop neofetch youtube-dl smartmontools bitwarden gparted htop kvantum-qt5 timeshift transmission-gtk vlc zsh oh-my-zsh zsh-theme-powerlevel10k python
...
## AUR Packages:
yay -S --noconfirm mailspring noisetorch-bin megatools-bin konsave
